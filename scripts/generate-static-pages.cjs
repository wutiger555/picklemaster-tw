        ...
        const INDEX_PAGES = [
            ...
            },
            {
                ...
                items: [
                    ...
                    { href: '/play', label: '揪團約打', sub: '只在報名、開團或按「我會去」時建立資料：暱稱、頭像樣式、自填程度、報名紀錄與一組隨機代號，存於 Cloudflare 雲端（境外），不蒐集姓名、email 或電話。IP 只以雜湊暫存 2 天用於防濫用。報名明細一年後自動刪除，可隨時在「我的球拍」自行刪除' },
