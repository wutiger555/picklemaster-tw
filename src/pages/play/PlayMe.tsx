    ...
    return (
      <div className="min-h-screen bg-neutral-50">
        <div className="container mx-auto max-w-lg px-4 pb-24 pt-6 md:pt-10">
          ...
          <h1 className="mt-4 text-2xl font-black text-neutral-900">我的團</h1>
          <p className="mt-2 text-neutral-600">你報名的團和你開的團都會列在這裡。現在還沒有：去大廳挑一團，第一次報名時取個暱稱就好，不用註冊。</p>
          ...
          <section className="mt-10 rounded-3xl bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-sm text-neutral-500">在舊手機的「我的團」頁最下面產生一組球友碼，輸入到這裡就能把紀錄轉過來。</p>
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      <div className="container mx-auto max-w-lg space-y-3 px-4 pb-24 pt-6 md:pt-10">
        ...
        <h1 className="text-2xl font-black text-neutral-900">我的團</h1>

        <section className="rounded-3xl bg-white p-4 shadow-sm">
          <h2 className="mb-2 font-black text-neutral-900">即將開打</h2>
          {games === null ? (
            ...
            <p className="text-sm text-neutral-500">目前沒有即將開打的團。<Link to="/play/" className="text-teal-700 underline">回大廳挑一團</Link>，或<Link to="/play/?new" className="text-teal-700 underline">自己開一團</Link>。</p>
          )}
        </section>

        <section className="rounded-3xl bg-white p-5 shadow-sm">
          <h2 className="mb-3 font-black text-neutral-900">我的球友名片 <small className="font-normal text-neutral-500">報名時別人看到的樣子</small></h2>
          <div className="flex items-center gap-4">
            <div className="grid justify-items-center">
              <PaddleAvatar name={me.nickname} seed={me.avatarSeed} size={52} />
              <button type="button" onClick={() => save({ avatarSeed: me.avatarSeed + 1 }, '換好新頭像了')} className="mt-1 text-xs font-bold text-teal-700">↻ 換頭像</button>
            </div>
          ...
          <p className="mb-1.5 mt-4 text-xs font-bold text-neutral-500">程度（只用來配對）</p>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            <div className="rounded-2xl bg-neutral-50 p-3 text-center"><b className="block font-mono text-2xl font-extrabold">{stats?.played ?? '–'}</b><small className="text-xs text-neutral-500">打過的場次</small></div>
            <div className="rounded-2xl bg-neutral-50 p-3 text-center"><b className="block font-mono text-2xl font-extrabold">{stats?.hosted ?? '–'}</b><small className="text-xs text-neutral-500">開過的團</small></div>
          </div>
          <small className="mt-3 block text-xs text-neutral-400">不用註冊，身分存在這支手機的瀏覽器裡。</small>
        </section>
        ...
        <section className="rounded-3xl bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-xs text-neutral-500">在新手機打開「我的團」，輸入這組碼就能轉移。每組只能用一次，重新產生後舊的就失效。</p>
        </section>
